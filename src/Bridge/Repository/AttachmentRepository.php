...
final class AttachmentRepository extends EntityRepository
{
    ...
    public function getFeaturedImage(int $postId): Attachment
    {
        $result = $this->entityManager->getConnection()->createQueryBuilder()
            ...
            ->addSelect("MAX(Case WHEN pm_self.meta_key = '_wp_attached_file' THEN pm_self.meta_value END) attached_file")
            ->addSelect("MAX(Case WHEN pm_self.meta_key = '_wp_attachment_metadata' THEN pm_self.meta_value END) attachment_metadata")
            ->from($this->entityManager->getTablesPrefix() . 'posts', 'p')
            ->join('p', $this->entityManager->getTablesPrefix() . 'postmeta', 'pm_parent', 'p.ID = pm_parent.meta_value')
